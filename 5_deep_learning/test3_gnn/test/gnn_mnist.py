...
# -*- coding: utf-8 -*-
# Author    : LuoXianchao
# Datetime  : 2023/12/21 19:59
# Module    : gnn_mnist.py
# explain   : https://github.com/DebasmitaGhose/PyTorch_Graph_Neural_Network_MNIST
...
class BorisGraphNet(nn.Module):
    def __init__(self, img_size=28, pred_edge=False):
        ...
        if pred_edge:
            ...
            self.pred_edge_fc = nn.Sequential(nn.Linear(4, 64),nn.ReLU(),
...
def test2(args, model, device, test_loader):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def main():
    ...
    print(f'WARNING: 模型已保存，存储路径为：{os.path.dirname(__file__)}')
    return model
